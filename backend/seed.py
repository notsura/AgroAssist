...
# Comprehensive Biological Intelligence Data
PESTS_DATA = [
    {
        "name": "Stem Borer",
        "type": "Insect",
    ...
    },
    {
        ...
        "symptoms": "Curling of leaves and presence of green/black tiny insects; clusters on spikes.",
        "causes": "Small sucking insects (Lipaphis erysimi) extracting sap from tender parts.",
        "risk_stage": "Flowering to Pod/Grain Formation",
        "prevention": "Early sowing and light trap usage; maintain bird populations.",
        "treatment": "Spray Neem oil or Imidacloprid during evening."
    },
    {
        "name": "Fall Armyworm",
        "type": "Insect",
    ...
    },
    {
        ...
        "type": "Fungal",
        "risk_level": "High",
        "image": "/pests/Early Blight.jpg",
        "symptoms": "Concentric rings on lower leaves; yellowing and defoliation.",
        "causes": "Alternaria solani fungus thriving in high humidity.",
    ...
    },
    {
        ...
        "type": "Viral",
        "risk_level": "High",
        "image": "/pests/Yellow Mosaic Virus.jpg",
    ...
    },
    {
        ...
        "type": "Fungal",
        "risk_level": "High",
        "image": "/pests/Tikka Leaf Spot.jpg",
    ...
    },
    {
        ...
        "type": "Insect",
        "risk_level": "High",
        "image": "/pests/Tobacco Caterpillar.jpg",
    ...
    },
    {
        ...
        "type": "Fungal",
        "risk_level": "High",
        "image": "/pests/Apple Scab.jpg",
        "symptoms": "Olive-green to brown spots on leaves and fruit; scabby lesions on skin.",
        "causes": "Venturia inaequalis fungus in cool, moist springs.",
        "risk_stage": "Bud Break to Fruit Maturity",
        "prevention": "Plant resistant varieties and maintain open canopies for airflow.",
        "treatment": "Apply sulfur-based fungicides or Captan."
    }
]
...
def seed_data():
    # 1. Seed Pests and create mapping
    db.pests.delete_many({})
    db.pests.insert_many(PESTS_DATA)
    pest_map = {p['name']: p['_id'] for p in db.pests.find()}
    print(f"Seeded {len(PESTS_DATA)} standalone pests.")

    # 2. Process Crops to resolve pest references
    processed_crops = []
    for crop in CROP_ROUTINES_DATA:
        if 'pests_diseases' in crop:
            # Map names to IDs
            pests_ids = []
            for p in crop['pests_diseases']:
                p_name = p['name'] if isinstance(p, dict) else p
                if p_name in pest_map:
                    pests_ids.append(pest_map[p_name])
            crop['pests_diseases'] = pests_ids
        processed_crops.append(crop)

    # 3. Seed Crops
    db.crops.delete_many({})
    db.crops.insert_many(processed_crops)
    print(f"Seeded {len(processed_crops)} crops with relational biothreat links.")
